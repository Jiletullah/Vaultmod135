...
NGame = {
	...
	RNW_MERC_COMPANY_CAVALRY_MIN = 0, 	-- min cavalry_weight if we're using cavalry
	RNW_MERC_COMPANY_CAVALRY_MAX = 0, 	-- max cavalry_weight if we're using cavalry
	RNW_MERC_COMPANY_ARTILLERY_MIN = 0, 	-- min artillery_weight if we're using artillery
	RNW_MERC_COMPANY_ARTILLERY_MAX = 0, 	-- max artillery_weight if we're using artillery
...
NDiplomacy = {
	...
	UNCONDITIONAL_SURRENDER_MONTHS = 6,				-- Months before unconditional surrender starts having an effect on Call for Peace. Set to negative values to disable feature.
	SHAREMAP_PRESTIGE_TRANSFER_LOSE = -10,				-- Prestige transferred for the Request to Share Maps diplomatic action.
	SHAREMAP_PRESTIGE_TRANSFER_GAIN = 5,				-- Prestige transferred for the Request to Share Maps diplomatic action.
	...
	ABANDON_UNION_PRESTIGE = -15,					-- The change in prestige for the overlord when abandoning a personal union.
	...
	UNCONDITIONAL_SURRENDER_MIN_MONTHS = 1,
},
...
NCountry = {
	...
	DISINHERIT_PRESTIGE_THRESHOLD = -100,
	...
	LIBERTY_DESIRE_HORDE = 0,						-- Liberty desire from vassal being a horde
...
NEconomy = {
	...
	MIN_DIPLO_LOAN_INTEREST = 0.1,
	MAX_DIPLO_LOAN_INTEREST = 5.0,
	DIPLO_LOAN_INTEREST_STEP = 0.1,
	MIN_DIPLO_LOAN_DURATION = 12,
	...
	CARAVAN_FACTOR = 12.0,							-- Development is divided by this factor, do not set to zero!
...
NMilitary = {
	...
	CAVALRY_SPEED = 0.85, 							-- _MDEF_CAVALRY_SPEED = 10,
	...
	TRANSPORT_COST = 2,	 							-- _MDEF_TRANSPORT_COST = 10,
	...
	NOMAD_LOOT_TRADITION = 0.1,	 				-- _MDEF_NOMAD_LOOT_TRADITION_;Military tradition a horde gets from looting territory
	...
	MORALE_RECOVERY_SPEED_OWN_TERRITORY = 0.1,		-- How much the recovery speed for morale will be modified if unit is in controlled territory
	...
	OVERRUN_FACTOR = 12,							-- How much stronger one side of a battle must be to cause the other side to be overrun (integer)
...
NAI = {
	...
	MAX_CAV_PERCENTAGE = 10, -- For modding, actual ratio is dynamically computed but will be no higher than this.
	...
	PS_SHORT_TERM_POOL = 150, -- Max power AI will store in its short-term spending pool
	...
	MISSION_PICK_CHANCE = 0, -- Monthly chance of AI picking a mission if it lacks one (of 100)
	...
	ARTILLERY_FRACTION = 0.1, 	-- Ratio of artillery to infantry AI will build (doubles after FORCE_COMPOSITION_CHANGE_TECH_LEVEL) (reduced when on tight budget)
	...
	COLONY_BUDGET_FRACTION = 1.0, -- AI will spend a maximum of this amount of monthly ducats for colonies.
	...
	FORCE_MARCH_MIN_SIZE = 30, -- AI will not force march units with less regiments than this
	...
	DEBASE_THRESHOLD = 100,-- AI will not debase if it has more gold than this.

	DEVELOPMENT_CAP_BASE = 21,	-- AI will not develop provinces that have more development than this or DEVELOPMENT_CAP_MULT*original development (whichever is bigger)
...
NNationDesigner = {
	...
	CONSORT_MIN_AGE = 20,
	CONSORT_MAX_AGE = 40,
	RULER_BASE_SKILL = 1,					-- This ruler skill level is free.
	RULER_MAX_SKILL = 6,
